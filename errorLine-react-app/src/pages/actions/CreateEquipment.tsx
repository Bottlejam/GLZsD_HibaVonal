...
const CreateEquipment: React.FC = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData({ ...formData, [name]: name === "stock" || name === "price" || name === "locationId" || name === "dormitoryId" ? Number(value) : value });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/Equipment/Admin/Create/Equipment`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setFormData({ name: "", stock: 0, price: 0, locationId: 0, dormitoryId: 0 });
      } else {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <label>Kollégium ID:</label>
          <input type="number" name="dormitoryId" value={formData.dormitoryId} onChange={handleChange} required />
        </div>
