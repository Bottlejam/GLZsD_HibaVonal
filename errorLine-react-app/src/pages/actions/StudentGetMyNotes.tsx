...
const StudentGetMyNotes: React.FC = () => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/Note/Student/Get/MyNotes`, {
        headers: {
        ...
        <ul>
          {notes.map((note) => (
            <li key={note.id}>
              ...
              <p><strong>Hiba ID:</strong> {note.issueId}</p>
              <hr />
            </li>
          ))}
        </ul>
...
export default StudentGetMyNotes;
