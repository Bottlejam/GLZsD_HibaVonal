...
const MaintenanceManagerGetAllNotes: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/Note/MaintenanceManager/Get/AllNotes`, {
          method: "GET",
          headers: {
        ...
        if (response.ok) {
          ...
          setError(result.message || "Hiba történt a jegyzetek lekérdezésekor.");
        }
      } catch (err) {
...
export default MaintenanceManagerGetAllNotes;
